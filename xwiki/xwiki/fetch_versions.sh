...
readonly ORG='xwiki'
readonly PROJ='xwiki-platform'
readonly TAGS_COUNT='3'
readonly STRIP_VERSION_PREFIX='true'

declare -a IGNORE_VERSIONS=()

readonly VERSION_REGEX='^xwiki-platform-(16|17|18)\.[0-9]+\.[0-9]+$'
...
get_latest_tags_by_major()
{
    ...
    majors=$(printf '%s\n' "$tags" \
        | sed -n 's/^xwiki-platform-\([0-9]\+\)\..*/\1/p' \
        | sort -n \
        | uniq)

    for major in $majors; do
        printf '%s\n' "$tags" \
        | grep -E "^xwiki-platform-${major}\.[0-9]+\.[0-9]+$" \
        | sort -V \
        | tail -"$TAGS_COUNT"
    done \
    | sort -V \
