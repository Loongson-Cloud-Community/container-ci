...
prepare()
{
    ...
    local ollama_bin="$PROJ-$version-linux-loongarch64.tar.zst"
    if [ ! -f "$ollama_bin"  ]; then
        wget -O "$ollama_bin" --quiet --show-progress "https://github.com/loongarch64-releases/ollama/releases/download/v$version/$PROJ-linux-loongarch64.tar.zst"
    fi
    tar -xvf "$ollama_bin" -C "$context"
