...
# 1. 从 versions.json 读取权威版本号（versions.sh 可能已更新为最新版本）
full_version=$(jq -r ".[\"$major_version\"].version // empty" "$versions_json")
if [ -z "$full_version" ]; then
    log ERROR "Version $major_version not found in $versions_json"
    exit 1
fi
...
# 2. 获取该版本的所有变体（例如 forky, slim-forky, alpine3.24）
